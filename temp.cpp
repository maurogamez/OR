...
void solve(IloModel); //res=1 implies solution obtained whereas res=0 implies infeasible
...
{	
	...
	try	
	{
		...
		//Alternate File Reading in comments
		/*if (argc > 1)
		{
...
/*----------------------------------------END OF MASTER PROBLEM-------------------------------------------------------------------------*/
	  ...
	  //xcolumns.setSize(S*M);
	  //ycolumns.setSize(S*M);
...
/*---------------------------------------------SOME INITIAL DECLARATIONS FOR YCOLUMN - END -------------------------------------*/
		...
		cout<<"\n\nSolved relaxed lp once completely!"<<endl;
		//return 0;
		/*Branch and bound*/
		...
		if(vals.areElementsInteger())
		{
			...
			env.out()<<"\n\nFinal incumbent solution: "<<Col_cost<<endl;
		...
		eps = cplex_master.getParam(IloCplex::EpInt);
		
		/*Queue of problems*/
		queue<IloModel> mod;
		mod.push(model_master);
		if(min_obj==1)
			incumbent = MAX;
		else incumbent = -MAX;
		//var.add(X);
		...
		system("pause");
		branch(mod, incumbent, cplex_master, Col_select);
...
void solve(IloModel model_master)
{
	res=0;
	RC_FLAG=1;
	...
	while (RC_FLAG)
	{
		 ...
		 for (IloInt r=0; r<R; r++)
		 {
			  ...
			  {//start y
				  ...
				  system("pause");
				  //cout<<0<<":"<<S<<":"<<M<<":"<<N<<":"<<R<<":"<<r<<":"<<j<<":"<<dist<<":"<<reduced_cost_y_column<<endl;
				  ...
				  int y_node_output = 0; // r-j semi-assignment solution number(k) for (k+1)th best
...
{		
	...
	//IloNumArray vals(env);
	cplex_master.getValues(vals, Col_select);
	//cplex_master.exportModel("trial.lp");
	cout<<"Number of variables: "<<var.getSize()<<endl;
	//env.out()<<var<<endl;
	env.out()<<vals<<endl;
	//getch();
	...
	value=vals[i];
	cout<<"Non-integral value: "<<vals[i]<<endl;
	IloNum objVal;
	cout<<"Written Master model"<<endl;
	cplex_master.exportModel(left.c_str());
	system("pause");
	/*Branch Left->lower bound*/
	IloModel lb(model);
	cout<<"Check1 var[i]: "<<endl;
